...
These functions are required inside a market/certificate contract in order for this contract to interface
*/
interface QMSI721 {
...
abstract contract ERC20Spendable is ERC20 {
  ...
  */
  ...
  /**
   * @dev Function to return rate at which funds should burn
   * @return A percent value between 0 - 100 of funds in circulation
   */
 ...
 /**
   ...
   * @param value The funds to take away from the pool
  */
...
contract QMSI_20 is ERC20, ERC20Spendable {
  ...
  function maxSupply() public view virtual returns (uint256) {
    ...
    */
    ...
    function getQNS(address account) public view virtual returns (uint256) {
      require(_QNS[account] > 0, "ERC20: No QNS set");
      return _QNS[account];
    }
    ...
    */
    function stake(uint256 days_, uint256 value_) external{
      ...
      require(balanceOf(msg.sender) > value_, "QMSI-ERC20: Not enough funds to lock");
      require(_Staked[msg.sender] == 0, "QMSI-ERC20: Cannot be staking funds already");
      require(reward + totalSupply() + totalStaked() < maxSupply(), "QMSI-ERC20: Reward exceeds total supply");
      require(reward < burnPool(), "QMSI-ERC20: Not enough funds to reward user from the burn pool");
    ...
    /**
    * @notice allows for user to unlock funds locked using stake function
    */
    function unlockFunds() external{
      require(_Staked[msg.sender] > 0, "QMSI-ERC20: Not staking any funds to unlock");
      require(block.timestamp > _Unlocker[msg.sender], "QMSI-ERC20: Funds are still locked");
    ...
    /**
     * @notice for checking the date an account is allowed to claim locked funds
     * @param account the address of the account that is staking an amount
     * @return The date of when the funds can be claimed back
     */
   ...
   * @param to the address of who we're sending funds to
  ...
  */
  function crossTokenBuy(address market, address to, uint256 tokenId, uint256 tokenPrice_, uint256 tokenCommission_) public returns (bool) {
    //require(isSpender(market) == true, "QMSI-ERC20: Market address must be spender");
    require(balanceOf(msg.sender) >= tokenPrice_, "QMSI-ERC20: Insufficient funds");
    ...
    require(tokenCommission_ == QMSI721(market).tokenCommission(tokenId), "ERC721: Commission rate does not match");
    require(bytes(QMSI721(market).tokenURI(tokenId)).length > 0, "ERC721Buy: Nonexistent token");

    require(QMSI721(market).tokenPrice(tokenId) > 0, "ERC721Buy: Token not for sale");
    require(msg.sender != QMSI721(market).ownerOf(tokenId), "ERC721Buy: Cannot buy your own token");

    // To prevent price manipulation by making user aware of the price by including it in the function call
    require(tokenPrice_ == QMSI721(market).tokenPrice(tokenId), "ERC721Buy: Price is not equal");

    require(to == QMSI721(market).ownerOf(tokenId), "ERC721: Sending funds to the wrong owner!");
